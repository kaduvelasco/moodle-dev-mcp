...
> ⚠️ **Known bug in `gemini mcp add` (v0.39.x):** the command may generate an invalid `settings.json` — `command` is saved as an empty string and `args` are not split correctly. After running it, **verify the generated file** and fix it if needed (see [Troubleshooting](#️-troubleshooting)).
>
> Prefer **Option 2** for a more reliable configuration.

### Option 2 — Editing settings.json directly (recommended)
...
### Server shows as "Disconnected" after `gemini mcp add`

Likely cause: a bug in `gemini mcp add` (v0.39.x) that generates an invalid `settings.json`.

Open `.gemini/settings.json` in the project directory (or `~/.gemini/settings.json` if you used `--global`) and check:

```json
// ❌ Incorrectly generated by the CLI
{
  "mcpServers": {
    "moodle-dev-mcp": {
      "command": "",
...
Fix it manually using one of these forms:
...
After fixing, restart the `gemini` session.
