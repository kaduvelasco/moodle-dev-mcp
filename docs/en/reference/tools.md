...
| `outputDir` | string | Directory where the ZIP will be saved. Defaults to the current working directory (optional) |

**What it does:** reads the version from `version.php` (`$plugin->version`), creates a ZIP named `{component}_{version}.zip` (e.g. `local_caedauth_2026041000.zip`) containing a single root folder named after the plugin directory (e.g. `caedauth/`). Empty directories are not included. Saves to `outputDir` if provided, otherwise to the current working directory.
...
> If any of these files or directories do not exist in the plugin, they are silently skipped — no error is raised.
